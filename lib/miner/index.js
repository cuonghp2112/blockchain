...
class Miner {
    ...
    static generateNextBlock(rewardAddress, feeAddress, blockchain) {
        ...
        // Get the new state of contract
        // R.forEach((tran)=>{
        //     if(tran.type=='contracttrans') contracttran.push(tran);
        // },transactions);
        var addr=[];
        let selectTxs = (tran)=>{
            ...
            R.forEach((txOutput)=>{
                ...
                addr.push(tran.getInputadd());
            }
            },tran.data.outputs);
            };
        };
        R.forEach(selectTxs,transactions);
                    ...
                    outputs: [
                        {
                            ...
                            dumb: addr, // INFO: Usually here is a locking script (to check who and when this transaction output can be used), in this case it's a simple destination address 
                        }
                    ]
                }
            });
