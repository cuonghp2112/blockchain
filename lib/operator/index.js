...
class Operator {
    ...
    createWallet(name,password) {
        let newWallet = Wallet.fromPassword(name,password);
    ...
    getWalletIdByName(name) {
        let wallet = R.find((wallet) => { return wallet.name == name; }, this.wallets);
        return wallet.id;
    }
    ...
    createcontractTransaction(walletId, fromAddressId, ctAddressId, content, changeAddressId) {
        let utxo = this.blockchain.getUnspentTransactionsForAddress(fromAddressId);
        let wallet = this.getWalletById(walletId);
        ...
        if (secretKey == null) throw new ArgumentError(`Secret key not found with Wallet id '${walletId}' and address '${fromAddressId}'`);
        var type = 'contractrans';
        let tx = new TransactionBuilder();
        tx.from(utxo);
        tx.toContract(ctAddressId, content);
        tx.change(changeAddressId || fromAddressId);
        tx.fee(Config.FEE_PER_TRANSACTION);
        tx.sign(secretKey);

        return Transaction.fromJson(tx.buildCallContractfunctran());
    }

    // getContractState(){
    //     let lastBlock = blockchain.getLastBlock();
    //     let candidateState = lastBlock.candidateState;
    //     R.forEach((candidate)=>{
    //         txCandidate.push({
    //             name
    //         })
