...
class Block {
    ...
    getContractstate(){
        let contract = new Contractaccounts();
        contract=this.contractstate;
        return contract;
    }
    getContractstateByAddress(address){
        // let findcontract = new Contractaccount();
        // R.forEach((contract)=> {if(contract.getAddress() == address) findcontract = contract},this.contractstate)
        // return findcontract;
        return R.find(R.propEq('address', address), this.contractstate);
    }
        ...
        R.forEachObjIndexed((value, key) => {
            if (key == 'transactions' && value) {
                block[key] = Transactions.fromJson(value);
                 ...
                 if(key == 'contractstate' && value){
                block[key] = Contractaccounts.fromJson(value)
            }
            else {
                block[key] = value;
            }
