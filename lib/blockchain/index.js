...
const ContractVoting = require('../contract');
...
class Blockchain {
    ...
    getContractstatebyaddress(address){
        let block = R.last(this.blocks);
        return block.getContractstateByAddress(address);
    }
    ...
    replaceChain(newBlockchain) {
        ...
        var i=this.blocks.length-1;
        while(i>=0){
            if(this.blocks[i].hash==newBlockchain[i].hash && this.blocks[i].previousHash==newBlockchain[i].previousHash){
                break;
            }
            i=i-1;
        }
        while(this.blocks.length-1>i){
            this.blocks.pop();
        }
        let newBlocks = R.takeLast(newBlockchain.length - this.blocks.length, newBlockchain);
...
//         let contractstate = [];  
//         let state = previousBlock. getContractstateByAddress('3as5f4a3d5sf46ad54fs6f');
//         let newstate = newBlock. getContractstate('3as5f4a3d5sf46ad54fs6f');
//         let votingcontract = new ContractVoting();
//         votingcontract.update(state.getVariable());
...
//         let selectTxs = (tran)=>{
//             if(tran.type == 'contracttrans'){
//             R.forEach((txOutput)=>{
//                 if(txOutput['address']=='3as5f4a3d5sf46ad54fs6f') {votingcontract.vote(tran.getInputadd(),txOutput.content);
//             }
//             },tran.data.outputs);
//             };
//         };
//         R.forEach(selectTxs,newBlock.transactions);
//         state.updateVariable(votingcontract.getstate());
//         contractstate.push(state);
//         if(!R.equals(contractstate,newstate)) {
//             console.error(`Invalid contractstate`);
//             throw new BlockAssertionError(`Invalid contractstate`);
//         }
    ...
    getOutputTransactionsForAddress(address) {	
        let indexOutput = 1;	
        const selectTxs = (transaction) => {	
            // Create a list of all transactions outputs found for an address (or all).	
            R.forEach((txOutput)=>{	
                ...
                R.forEach((txInput) => {	
                    if(flag==0){	
                        if (txInput.address == address && txOutput.address != txInput.address) {	
                            txOutputs.push({	
                                transaction:transaction.id,	
                                index: indexOutput,	
                                amount: txOutput.amount,	
     ...
     getInputTransactionsForAddress(address) {	
        let indexInput = 1;	
        const selectTxs = (transaction) => {     	
            R.forEach((txOutput)=>{	
                let flag = 0;	
                R.forEach((txInput) => {	
                    if(flag==0){	
                        if (txOutput.address == address && txOutput.address != txInput.address) {	
                            txInputs.push({	
                                transaction: transaction.id,	
                                index: indexInput,	
                                amount: txOutput.amount,	
